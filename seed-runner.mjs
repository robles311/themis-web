...
/**
 * seed-runner.mjs
 * Self-contained seed script — connects to pgvector + DeepSeek.
 * Run inside the Docker app container or any env with DATABASE_URL + DEEPSEEK_API_KEY.
 *
 * Usage: node seed-runner.mjs
 */
...
async function run() {
  ...
  for (const doc of DOCUMENTS) {
    process.stdout.write(`  Embedding ${doc.specialty} / ${doc.code_name} Art.${doc.article_number}... `);
  ...
  console.log(`\n✅ Seeded ${inserted} documents across ${new Set(DOCUMENTS.map(d => d.specialty)).size} specialties`);
  const counts = {};
  for (const d of DOCUMENTS) counts[d.specialty] = (counts[d.specialty] || 0) + 1;
